...
class QbExamsListView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final activeTab = activeExamTab ?? 'সব';
        ...
        if (hasMultipleLevels) ...[
          Padding(
            ...
            child: SizedBox(
              height: 44,
              child: TextField(
                onChanged: onExamSearchQueryChanged,
                style: TextStyle(
                  fontSize: 13,
                  color: isDark ? Colors.white : Colors.black87,
                  fontFamily: 'Noto Sans Bengali',
            ...
            builder: (context, ref, _) {
              ...
              return examsAsync.when(
                data: (examsList) {
                  ...
                  if (hasMultipleLevels && examSearchQuery.isNotEmpty) {
                    filteredList = examsList.where((ex) {
                      final title = ex['title']?.toString() ?? '';
                      return title.toLowerCase().contains(examSearchQuery.toLowerCase());
                    }).toList();
                  }
